...
function createFiles(templateFolder, name, template, vars, outputdir, done) {
  ...
  if (!defJson[template]){
    done('template not found', null);
  }
  const defJsonVars = defJson[template].vars;
  const defJsonBundle = defJson[template].bundle;
  const defJsonFiles = defJson[template].files;
